...
class BaseInception(nn.Module):
    def __init__(self, in_channel, out_channel_list, reduce_channel_list):
        ...
        self.branch1_conv = ConvBNRelu(in_channel=in_channel,
                                        out_channel=out_channel_list[0],
                                        kernel_size=1)

        self.branch2_conv1 = ConvBNRelu(in_channel=in_channel,
                                        out_channel=reduce_channel_list[0],
                                        kernel_size=1)
        self.branch2_conv2 = ConvBNRelu(in_channel=reduce_channel_list[0],
                                        out_channel=out_channel_list[1],
                                        kernel_size=3)

        self.branch3_conv1 = ConvBNRelu(in_channel=in_channel,
                                        out_channel=reduce_channel_list[1],
                                        kernel_size=1)
        self.branch3_conv2 = ConvBNRelu(in_channel=reduce_channel_list[1],
                                        out_channel=out_channel_list[2],
                                        kernel_size=5)
        ...
        self.branch4_conv = ConvBNRelu(in_channel=in_channel,
                                        out_channel=out_channel_list[3],
                                        kernel_size=3)
    def forward(self, x):
...
class InceptionNet(nn.Module):
    ...
    def __init__(self):
        ...
        # 256 = 64 * 4 from  above out_channel_list=[64, 64, 64, 64]
